...
/** 
 * Gets or set the reader oject
 * @param Object reader
 * @return {Object or Metalsmith}
 */
 Metalsmith.prototype.reader = function(reader) {
  if (!arguments.length) return this._reader;
  assert(is.object(reader) && is.function(reader.readDir) && is.function(reader.readFile), 
        'The reader object must have readDir and readFile functions.');
  this._reader = reader;
  return this;
 };
...
Metalsmith.prototype.read = unyield(function*(dir){
  ...
  var paths = yield this._reader.readDir(dir, ignores);
  var files = [];
  var complete = 0;
  var batch;
...
Metalsmith.prototype.readFile = unyield(function*(file){
  ...
  try {
    ...
    var stats;
    if (this._reader.stat) { 
      yield this._reader.stat(file);
    }
    var buffer = yield this._reader.readFile(file);
    var parsed;
